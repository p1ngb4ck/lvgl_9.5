...
async def to_code(configs):
    ...
    if CORE.build_path:
        ...
        cmake_content = "# Auto-generated by lvgl component\n"

        # Deferred: patch lv_freertos.c and remove excluded sources from __idf_lvgl__lvgl
        # after idf_component_register — managed_components/ exists only at this point.
        patch_cmake = ""
        if patch_src.is_file():
            patch_cmake = (
                f'  message(STATUS "lvgl: applying lv_freertos.c patch (IDF 6.x SMP + internal SRAM stack)")\n'
                f'  execute_process(\n'
                f'      COMMAND ${{CMAKE_COMMAND}} -E copy\n'
                f'          "{dst_patch.as_posix()}"\n'
                f'          "{lv_freertos_dst}"\n'
                f'      RESULT_VARIABLE _lvgl_patch_result\n'
                f'  )\n'
                f'  if(_lvgl_patch_result EQUAL 0)\n'
                f'    message(STATUS "lvgl: lv_freertos.c patch applied OK")\n'
                f'  else()\n'
                f'    message(WARNING "lvgl: lv_freertos.c patch FAILED result=${{_lvgl_patch_result}}")\n'
                f'  endif()\n'
            )
