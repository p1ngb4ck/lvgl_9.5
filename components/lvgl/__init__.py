    ...
    if CORE.build_path:
        ...
        if patch_src.is_file():
            cmake_content += (
                ...
                f'    OUTPUT "{lv_freertos_dst}"\n'
                f'    COMMAND ${{CMAKE_COMMAND}} -E copy\n'
                f'        "{dst_patch.as_posix()}"\n'
                f'        "{lv_freertos_dst}"\n'
                f'    DEPENDS "{dst_patch.as_posix()}"\n'
                f'    COMMENT "lvgl: applying lv_freertos.c patch (SMP deadlock fix + ESP-IDF adaptations)"\n'
                f'    VERBATIM\n'
                f')\n'
                f'add_custom_target(lvgl_freertos_patch ALL\n'
                f'    DEPENDS "{lv_freertos_dst}"\n'
                f')\n'
            )
