...
async def to_code(configs):
    ...
    if CORE.build_path:
        ...
        if patch_src.is_file():
            ...
            _LOGGER.info(
                "lvgl: lv_freertos.c patch staged -> %s (SMP deadlock fix + ESP-IDF adaptations)",
                dst_patch,
            )
        ...
        if patch_src.is_file():
            cmake_content += (
                f'message(STATUS "lvgl: applying lv_freertos.c patch (SMP deadlock fix + ESP-IDF adaptations)")\n'
                f'execute_process(\n'
                ...
                f'    message(STATUS "lvgl: lv_freertos.c patch applied OK -> {lv_freertos_dst}")\n'
                f'else()\n'
                f'    message(WARNING "lvgl: lv_freertos.c patch FAILED (managed component not yet present?) result=${{_lvgl_patch_result}}")\n'
                f'endif()\n'
            )
